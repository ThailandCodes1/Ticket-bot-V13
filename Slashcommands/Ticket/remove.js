...
module.exports = {
  name:"remove",
  description:"➖ Remove member from the ticket",
  cooldown:15,
  options:[
    {
      name:"user",
      description:"The user to removed",
      type:"USER",
      required:true
    },
  ],
  ...
  run : async(interaction, client, args) => {
    ...
    if (user.user.id === interaction.user.id ) return interaction.editReply({content:"🤔 - You can't remove yourself"});
    //Role_1
    ...
    .setColor("RED")
    .setDescription(`${user} removed from ticket ${ch}`)
